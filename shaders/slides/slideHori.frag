...
void main() {
    ...
    float moveAmount = min(uTime * 0.75,1.0) * uReverse;
    // float moveAmount = min(uTime * 0.75,1.0) ;
    ...
    float slide2InBounds = step(0.0,uv2.x) * step(uv2.x,1.0) * step(borderLength,st.y) * step(st.y,1.0 - borderLength);
